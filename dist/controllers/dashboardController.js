...
const getDashboard = async (req, res) => {
    ...
    try {
        const { rows: stats } = await db_1.default.query("SELECT win_count, game_count FROM players WHERE id = $1", [player.id]);
        const { rows: activeGames } = await db_1.default.query(`SELECT g.id, g.name, g.status, g.created_at, g.creator_id, gt.name as game_type_name
            FROM games g
            ...
            ORDER BY g.created_at DESC`, [player.id]);
        const { rows: finishedGames } = await db_1.default.query(`SELECT DISTINCT ON (g.id) g.id, g.name, g.status, g.created_at, g.creator_id,
            ...
            ORDER BY g.id, totals.total DESC, g.created_at DESC`, [player.id]);
